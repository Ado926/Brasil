...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  if (coin > 10000) return conn.reply(m.chat, `✎  La cantidad máxima de apuesta es de 50 ${moneda}.`, m)

  if (!(color === 'black' || color === 'red')) return conn.reply(m.chat, `${emoji2} Debes apostar a un color válido: *black* o *red*.`, m)

  if (coin > users.coin) return conn.reply(m.chat, `《✧》 No tienes suficientes ${moneda} para realizar esa apuesta.`, m)
